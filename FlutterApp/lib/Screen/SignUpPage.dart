...
class _SignUpState extends State<SignUp> {
  ...
  registerUser() async{
    ...
    await Provider.of<Applicationbloc>(context, listen: false).signup(data, 'register').then((response){
      setState(() {
        loading = false;
      });
      if (response['status'] && Provider.of<Applicationbloc>(context, listen: false).registeredInStatus == Status.Registered) {
        Provider.of<UserProvider>(context, listen: false).setPhoneNumber(_phoneno.text.toString());
        UserPreferences().saveToken(fcmToken);
        print("fcmtaken =>$fcmToken");
      ...
      body: SafeArea(
        child: Container(
          ...
          child: Padding(
            ...
            child: loading ? Center(child: CircularProgressIndicator(),): Form(
              key: _signupformKey,
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: TextButton(
                        ...
                        child: Text('Sign Up', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w300),),
                        onPressed: () {
                          if(_signupformKey.currentState!.validate()){
                            registerUser(); print(applicationbloc.registeredInStatus);
                          }
                        },
                      ),
                    ),
